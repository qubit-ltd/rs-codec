...
use crate::{
    CapacityError, Codec, TranscodeEncodeError, TranscodeEncodeErrorOf, TranscodeStatus,
    Transcoder,
        ...
        M: FnMut(C::EncodeError) -> Error,
    {
        ...
        if let Err(error) = self.output.ensure_spare_capacity(max_units) {
            ...
            let written = map_encode_value_result(
                encode_complete_value_into_reserved(codec, value, &mut scratch, 0, max_units),
                &mut map_error,
            )?;
