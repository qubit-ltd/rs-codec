...
use crate::{
    Codec, DecodeLifecycleOutput, DecodeLifecycleProgress, TranscodeFailure, TranscodeProgress,
    TranscodeStatus, Transcoder, codec::assert_unit_bounds,
};
    ...
    I::Item: Copy + Default,
{
    ...
    ///
    /// The wrapped input can be physically positioned after units that remain
    /// unread in this adapter's internal buffer.
    ///
    /// # Returns
    ///
    ...
    /// # Errors
    ///
    /// Returns invalid output ranges, insufficient declared output capacity,
    /// capacity errors, or reset errors mapped by `map_error`.
    ///
    /// # Panics
    ///
    /// Panics when the decoder reports writing more values than its declared
    /// reset bound.
    ...
    /// # Returns
    ///
    /// The number of values written. Once a refill confirms EOF, any retained
    /// tail is passed to [`Transcoder::transcode_eof`] before this method
    /// returns.
    ///
    /// # Errors
    ///
        ...
        M: FnMut(D::Error) -> Error,
    {
        ...
        loop {
            ...
            let progress = decoder
                ...
                .map_err(&mut *map_error)?;
            let progress = validate_decode_progress(
                progress,
                0,
                available_input,
            ...
            match progress.status() {
                ...
                TranscodeStatus::NeedInput { required, .. } => {
                    ...
                    let remaining_output = count - written_total;
                    let eof_progress = self.transcode_eof_step(
                        decoder,
                        map_error,
                        output,
                        output_index + written_total,
                    ...
                    written_total += eof_progress.written();
                    return Ok(written_total);
                }
            }
        }
