...
use super::{encode_context::EncodeContext, encode_unencodable_action::EncodeUnencodableAction};
use crate::{CapacityError, Codec, TranscodeEncodeErrorOf};
    ...
    C: Codec,
{
    ...
    ///
    /// `finish` never receives more input values. Implementations must report
    /// the maximum output that could be derived from hook-owned state in any
