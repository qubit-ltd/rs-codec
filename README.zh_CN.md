...
- **类型化错误**：畸形输入返回 `CodecError::InvalidBase64`。
...
| `with_prefix(prefix)` | 在每个 byte 前添加并要求前缀，例如 `0x1F 0x8B` |
...
| 错误 | 含义 |
|------|------|
| `MissingPrefix` | 配置了逐字节十六进制前缀，但输入缺少该前缀 |
| `OddHexLength` | 规范化后的十六进制位数为奇数 |
| `InvalidHexDigit` | 十六进制输入包含非十六进制字符 |
| `InvalidBase64` | Base64 输入畸形 |
| `InvalidPercentEscape` | Percent 输入包含畸形 `%XX` 转义 |
