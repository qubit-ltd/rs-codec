...
use qubit_codec::{
    CapacityError, TranscodeDecodeError, TranscodeDecodeInput,
    TranscodeDecoder, TranscodeProgress, Transcoder,
};
...
impl Transcoder for MarkerDecoder {
    ...
    fn transcode(&mut self, input: &[u8], input_index: usize, output: &mut [u8], output_index: usize) -> Result<TranscodeProgress, Self::Error> {
        ...
        if output_index == output.len() {
            return Ok(TranscodeProgress::need_output(output_index, qubit_codec::nz(1), 0, 0, 0));
        }
...
impl TranscodeDecoder for MarkerDecoder { type DecodeError = Infallible; }
