...
fn test_codec_trait_encode_allows_buffered_zero_output() {
    ...
    let first = unsafe { codec.encode(&7, &mut output, 0) }
        .expect("buffering should not fail");
    let second = unsafe { codec.encode(&0, &mut output, 0) }
        .expect("flushing buffered value should not fail");
