...
fn test_lifecycle_reset_reopens_finished_stream() {
    ...
    assert_eq!(
        Err(TranscodeDecodeError::Failure(TranscodeFailure::FinishBeforeReset)),
        decoder.finish(&mut [], 0),
    );

    let mut output = [0_u8; 1];
    assert_eq!(
        Err(TranscodeDecodeError::Failure(TranscodeFailure::TranscodeBeforeReset,)),
        decoder.transcode(&[5], 0, &mut output, 0),
    );

    decoder.reset(&mut [], 0).expect("reset should initialize the stream");
    decoder
        .finish(&mut [], 0)
        .expect("empty stream should finish after reset");
    decoder.reset(&mut [], 0).expect("reset should reopen stream");
