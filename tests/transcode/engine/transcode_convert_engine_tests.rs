...
impl Codec for ErrorSourceCodec {
    ...
    unsafe fn decode(
        ...
        Err(codec::DecodeFailure::invalid_unknown(EngineError::Decode))
    }
...
impl TranscodeDecodeHooks<SourceCodec> for StrictDecodeHooks {
    ...
    ) -> Result<DecodeInvalidAction<u8>, codec::TranscodeDecodeErrorOf<SourceCodec>> {
        match error {
...
impl TranscodeDecodeHooks<ShortSourceCodec> for EofIncompleteDecodeHooks {
    fn handle_invalid_decode(
        ...
        _consumed: Option<NonZeroUsize>,
        _context: DecodeContext,
    ) -> Result<DecodeInvalidAction<u8>, codec::TranscodeDecodeErrorOf<ShortSourceCodec>> {
    ...
    ) -> Result<DecodeIncompleteAction<u8>, codec::TranscodeDecodeErrorOf<ShortSourceCodec>> {
        assert_eq!(None, source);
        assert_eq!(crate::nonzero(2), required_total);
        assert_eq!(1, context.available());
        match self.mode {
            EofIncompleteMode::Emit => Ok(DecodeIncompleteAction::Emit { value: 77 }),
...
impl TranscodeEncodeHooks<TargetCodec> for PerBatchOverheadEncodeHooks {
    ...
    ) -> Result<EncodeUnencodableAction<u8>, codec::TranscodeEncodeErrorOf<TargetCodec>> {
        Err(TranscodeEncodeError::domain_main(
            EngineError::Encode,
            context.input_index(),
        ))
...
impl<const FINISH_BOUND: usize> TranscodeDecodeHooks<FinishValueSourceCodec<FINISH_BOUND>>
    for FixedFinishBoundDecodeHooks
{
    fn max_finish_output_len(&self, _codec: &FinishValueSourceCodec<FINISH_BOUND>) -> usize {
        self.finish_len
    }
...
impl TranscodeDecodeHooks<SourceCodec> for FinishDecodeHooks {
    ...
    fn finish_hooks(
        ...
        if self.fail {
            return Err(TranscodeDecodeError::domain_finish(EngineError::Decode));
        }
...
impl TranscodeEncodeHooks<TargetCodec> for ErrorPathEncodeHooks {
    ...
    fn handle_unencodable_encode(
        ...
        match self.mode {
            ErrorPathEncodeMode::PrepareError => Err(TranscodeEncodeError::domain_main(
                EngineError::Encode,
                _context.input_index(),
            )),
...
fn test_transcode_convert_engine_rejects_incomplete_eof_tail() {
    ...
    assert_eq!(
        TranscodeConvertError::Failure(TranscodeFailure::incomplete_input(0, 2, 1)),
        error,
    );
}
...
fn test_buffered_convert_engine_maps_pending_encode_error_before_new_input() {
    ...
    assert!(matches!(
        error,
        TranscodeConvertError::EncodeDomain(codec::TranscodeDomainError::Main {
...
fn test_buffered_convert_engine_reports_invalid_indices() {
    ...
    assert_eq!(
        codec::TranscodeConvertError::Failure(codec::TranscodeFailure::invalid_output_index(2, 1)),
        error,
    );
}
...
fn test_buffered_convert_engine_reports_capacity_errors() {
    ...
    assert_eq!(
        codec::TranscodeConvertError::Failure(codec::TranscodeFailure::output_length_overflow()),
        error
    );
...
fn test_buffered_convert_engine_finish_reports_output_index_beyond_buffer() {
    ...
    assert_eq!(
        codec::TranscodeConvertError::Failure(codec::TranscodeFailure::invalid_output_index(1, 0)),
        error,
    );
}
...
fn test_buffered_convert_engine_finish_maps_decode_error() {
    ...
    assert!(matches!(
        error,
        TranscodeConvertError::DecodeDomain(codec::TranscodeDomainError::Finish {
...
fn test_buffered_convert_engine_finish_maps_encode_error() {
    ...
    assert!(matches!(
        error,
        TranscodeConvertError::EncodeDomain(codec::TranscodeDomainError::Finish {
            source: EngineError::Encode,
        }),
    ));
    assert_eq!([0], output);
...
fn test_buffered_convert_engine_reset_maps_target_reset_errors() {
    ...
    assert!(matches!(
        error,
        TranscodeConvertError::EncodeDomain(codec::TranscodeDomainError::Reset {
...
fn test_buffered_convert_engine_reset_maps_stateless_decoder_reset_error() {
    ...
    assert!(matches!(
        error,
        TranscodeConvertError::DecodeDomain(codec::TranscodeDomainError::Reset {
            source: EngineError::Decode
        }),
    ));
}
...
impl TranscodeDecodeHooks<FinishFailingSourceCodec> for FinishFailSourceDecodeHooks {
    fn handle_invalid_decode(
        &mut self,
