...
use qubit_codec::{
    CapacityError, Codec, CodecPhase, DecodeContext, DecodeInvalidAction, TranscodeDecodeEngine,
    TranscodeDecodeHooks, TranscodeError, TranscodeStatus, Transcoder,
};
...
fn test_transcode_decode_engine_reports_finish_bounds() {
    ...
    let max_total_output_len: fn(
        &Decoder,
        usize,
    ) -> Result<usize, TranscodeError<PrefixDecodeError>> = Decoder::max_total_output_len;
