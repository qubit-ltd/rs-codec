...
use qubit_codec::{
    CapacityError, TranscodeEncodeOutput, TranscodeError, TranscodeProgress, Transcoder,
};
use qubit_io::Output;
...
impl Transcoder<u32, u16> for PairEncoder {
    ...
    fn transcode(
        ...
        if output_index > output.len() {
            return Err(TranscodeError::Domain(PairEncodeError::BadOutputIndex));
        }
        let mut read = 0;
        let mut written = 0;
...
fn test_buffered_encode_output_rejects_overreported_read_progress() {
    ...
    assert_eq!(ErrorKind::InvalidData, error.kind());
    assert!(error.to_string().contains("consumed beyond input range"));
}
...
fn test_buffered_encode_output_rejects_overreported_write_progress() {
    ...
    assert_eq!(ErrorKind::InvalidData, error.kind());
    assert!(error.to_string().contains("wrote beyond spare output"));
}
...
fn test_buffered_encode_output_rejects_overflowing_need_output() {
    ...
    // SAFETY: The full input range is valid.
    let error = unsafe { encode_with(&mut output, &mut encoder, &[0x1234], 0, 1) }
        .expect_err("overflowing NeedOutput requirement should be rejected");

    assert_eq!(ErrorKind::InvalidData, error.kind());
    assert!(error.to_string().contains("output requirement overflowed"));
}
