...
fn test_transcoder_progress_exposes_status_and_counts() {
    ...
    assert_eq!(
        TranscodeStatus::need_input(4, nz(3), 1),
        need_input.status()
    );
    ...
    assert_eq!(
        TranscodeStatus::need_output(7, nz(8), 9),
        need_output.status()
    );
...
fn test_transcoder_progress_constructors_create_expected_progress() {
    let need_input = TranscodeProgress::need_input(4, nz(2), 1, 5, 6);
    assert_eq!(
    ...
    let need_output = TranscodeProgress::need_output(7, nz(3), 0, 8, 9);
    assert_eq!(
