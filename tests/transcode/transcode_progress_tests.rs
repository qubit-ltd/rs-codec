...
fn test_transcoder_progress_exposes_status_and_counts() {
    ...
    assert_eq!(
        TranscodeStatus::need_input(crate::nz(3)),
        need_input.status()
    );
    ...
    assert_eq!(
        TranscodeStatus::need_output(crate::nz(8)),
        need_output.status()
    );
...
fn test_transcoder_progress_constructors_create_expected_progress() {
    let need_input = TranscodeProgress::need_input(crate::nz(2), 5, 6);
    assert_eq!(
    ...
    let need_output = TranscodeProgress::need_output(crate::nz(3), 8, 9);
    assert_eq!(
...
fn test_transcoder_progress_validate_accepts_consistent_progress() {
    ...
    let need_input = TranscodeProgress::need_input(crate::nz(4), 2, 3);
    assert_eq!(Ok(()), need_input.validate(10, 3, 20, 5));

    let need_output = TranscodeProgress::need_output(crate::nz(4), 2, 3);
...
fn test_transcoder_progress_validate_ignores_redundant_status_indices() {
    let need_input = TranscodeProgress::need_input(crate::nz(3), 2, 0);
    assert_eq!(Ok(()), need_input.validate(10, 3, 20, 0),);
}

#[test]
fn test_transcoder_progress_validate_rejects_satisfied_requirements() {
    let need_input = TranscodeProgress::need_input(crate::nz(1), 2, 0);
    assert_eq!(
    ...
    let need_output = TranscodeProgress::need_output(crate::nz(1), 0, 3);
    assert_eq!(
        Err(TranscodeContractError::SatisfiedNeed {
            required: 1,
            available: 1,
...
fn test_transcoder_progress_validate_derives_available_capacity() {
    let need_input = TranscodeProgress::need_input(crate::nz(4), 2, 0);
    assert_eq!(Ok(()), need_input.validate(10, 3, 20, 0));

    let need_output = TranscodeProgress::need_output(crate::nz(4), 0, 3);
    assert_eq!(Ok(()), need_output.validate(10, 0, 20, 4));
}

#[test]
fn test_transcoder_progress_validate_rejects_index_overflow() {
    let need_input = TranscodeProgress::need_input(crate::nz(2), 1, 0);
    assert_eq!(
    ...
    let need_output = TranscodeProgress::need_output(crate::nz(2), 0, 1);
    assert_eq!(
        Err(TranscodeContractError::ProgressIndexOverflow {
            index: usize::MAX,
            advanced: 1,
