...
fn test_value_codec_descriptor_executes_both_directions() {
    ...
    assert_eq!(
        DESCRIPTOR.codec_type_name(),
        std::any::type_name::<U32Codec>()
    );
...
fn test_value_codec_descriptor_reports_type_and_domain_errors() {
    ...
    let encode = DESCRIPTOR
        .encode(&u32::MAX)
        .expect_err("fixture encode error");
    assert!(matches!(
        encode,
        ValueCodecExecutionError::EncodeFailed { .. }
    ));
    ...
    assert!(matches!(
        decode,
        ValueCodecExecutionError::DecodeFailed { .. }
    ));
}
...
fn test_value_codec_id_protocol() {
    assert_eq!(
        ValueCodecId::new("example.Codec_1").as_str(),
        "example.Codec_1"
    );
    assert_eq!(ValueCodecId::try_new(""), Err(ValueCodecIdError::Empty));
    assert_eq!(
        ValueCodecId::try_new("example."),
...
fn test_value_codec_registry_rejects_duplicate_ids() {
    let error =
        ValueCodecRegistry::from_registrations([&FIRST, &SECOND]).expect_err("duplicate ID");
...
fn test_global_value_codec_registry_initialization_is_unique_across_threads() {
    let addresses = (0..8)
        .map(|_| {
            std::thread::spawn(|| {
                ValueCodecRegistry::try_global().expect("valid global registry")
                    as *const ValueCodecRegistry as usize
            })
        })
